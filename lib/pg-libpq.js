...
PG.prototype = {
  ...
  finish: function () {
    ExecQ.clearQueue(this);
    ExecQ.queue(this, function (pgConn) {
      var pq = pgConn.pq;
      pgConn.pq = null;
      pq.finish();
    });
  ...
  exec: function (command, callback) {
    ExecQ.queue(this, function (pgConn, qItem) {
      pgConn.pq.execParams(command, null, handleCallback(qItem, callback));
    });
  },
  ...
  execParams: function (command, params, callback) {
    ...
    ExecQ.queue(this, function (pgConn, qItem) {
      pgConn.pq.execParams(command, params, handleCallback(qItem, callback));
    });
  },
...
function handleCallback(qItem, callback) {
  return function (err, result) {
    try {
      if (err) {
        var ex = new Error(err);
        ex.sqlState = qItem[1].pq &&
          qItem[1].pq.resultErrorField(ERROR_FIELDS.SQLSTATE);
        callback(ex);
      }
      else
